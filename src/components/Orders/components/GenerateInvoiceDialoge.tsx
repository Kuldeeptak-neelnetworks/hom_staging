...
const GenerateInvoiceDialoge = ({ id, orderNo, dateOfOrder }: any) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {/* Button Section  */}
      <DialogTrigger>
        <PlusIcon className="h-[1.35rem] w-[1.75rem] p-1 hover:bg-[#004d4b] text-black hover:text-[white] " />
      </DialogTrigger>
      {/* form Section  */}
      <DialogContent className="sm:max-w-[500px] text-[0.8rem]">
        <div className="flex justify-center text-xl text-[#004d4b]">
          <FileTextIcon className="h-7 w-7 p-1 hover:bg-[#004d4b] text-[#004d4b] hover:text-[white] " />{" "}
          Generate Invoice
        </div>
